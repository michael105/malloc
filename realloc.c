//+def
MF void* realloc_brk(void *p, ml_size_t newsize){
...
#define ASSERT(expr) 
	...
	if ( *CAST( m + oldsize ) == mlgl->cbrk ){
		if ( m + newsize >= (void*)mlgl->brk ){
			if ( !ml_malloc_brk_addmem(m+newsize+BRKSZ) ){
				return(0);
			}
		}

		*CUR = newsize;
		mlgl->cbrk = (typeof(mlgl->cbrk))(m + newsize);
		*CAST(mlgl->cbrk) = 0;
